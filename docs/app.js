...
function setDataLoad(mode, ok, count, error){
  ...
  if(!el) return;
  if(mode==='remote' && ok){
    el.textContent = `Data: remote JSON loaded (${count} records)`;
  }else{
...
function renderQuotes(rows){const x=tr();const sorted=[...rows].sort((a,b)=>b.score!==a.score?b.score-a.score:a.date.localeCompare(b.date));const shown=state.quoteMode==='top'?sorted.slice(0,8):sorted;$('quoteCount').textContent=shown.length + ' shown';$('quoteMeta').textContent=x.quotesShown(shown.length,rows.length);$('showTopBtn').classList.toggle('active',state.quoteMode==='top');$('showAllBtn').classList.toggle('active',state.quoteMode==='all');$('showTopBtn').classList.toggle('subtle-btn',state.quoteMode!=='top');$('showAllBtn').classList.toggle('subtle-btn',state.quoteMode!=='all');$('quotes').innerHTML=shown.length?shown.map(r=>`<details><summary><span>${r.source} &middot; ${r.community} &middot; ${useLabel(r.useCase)}</span><span class="metric-chip ${r.score>=4?'':r.score===3?'mid':'low'}">${x.score} ${r.score}</span></summary><p class="meta">${r.date}</p><blockquote>${r.excerpt}</blockquote><p><strong>${x.interp}:</strong> ${note(r)}</p><p><a href="${r.url}" target="_blank" rel="noreferrer">${x.openSource}</a></p></details>`).join(''):`<div class="item"><p>${x.noData}</p></div>`}
